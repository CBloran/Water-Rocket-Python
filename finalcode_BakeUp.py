...
def water_exit_velocity(k, p_in):
    """
        Calcule la vitesse d'éjection basée sur la différence de pression (Bernoulli).
        """
    delta_p = p_in - patm  # Pression manométrique
    if delta_p <= 0:
        return 0

    # Formule de Bernoulli : v = sqrt(2*delta_P / rho)
    # Le terme (1 - (Ae/A)**2) est un correctif de continuité
    v_e = math.sqrt((2 * delta_p) / (rho_w * (1 - (Ae / A) ** 2)))
    return v_e


def equation_vel(v, Vw):
    mw_current = rho_w * Vw  # Masse d'eau restante
    m_totale = mb + mw_current

    F_drag = Drag(v)
    # On s'assure que la traînée s'oppose toujours au mouvement
    ...
    # La division par m_totale (qui varie) est la clé ici
    dv_dt = (F_thrust - F_drag - F_weight) / m_totale
    return dv_dt
...
def txtGraph(xs: list, ys: list, FileName="Output.txt"):
    ...
    for i in range(
            len(x_simple)):  # only keep the rounded value of our data because we cant use decimal number to set the position of the text
...
def Rungekutta(stepNbr=3000):
    ...
    while i < stepNbr:
        ...
        # Calcul de la nouvelle position/vitesse/volume
        ...
        # condition d'arret : Si on touche le sol (h <= 0) après avoir décollé (i > 10)
        if i > 10 and y_new[0] <= 0:
            break
